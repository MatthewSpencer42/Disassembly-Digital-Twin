...
class FlipDropSkill(Node):
    ...
    def execute_flip_drop(self, interactive=False):
        ...
        if not self.uf850.move_to_pose_exotica(self.INTERMEDIATE_POSE['x'], self.INTERMEDIATE_POSE['y'], self.INTERMEDIATE_POSE['z'], q_start, velocity=0.1): return False
        self.wait_for_arm_settled()
        ...
        if orig_j6 > 0: joints["uf850_joint6"] = orig_j6 - math.pi
        else: joints["uf850_joint6"] = orig_j6 + math.pi
        ...
        if not self.uf850.move_to_pose_exotica(sx, sy, sz + self.RETRACT_Z_HEIGHT, q_start, velocity=0.1): return False
        self.wait_for_arm_settled()

        # 6. TACTILE DESCENT
        print("⬇️ STEP 6: EXOTica stepped tactile descent...")
        if not self.uf850.move_linear_z_with_effort_stop_exotica(
            descent_distance_m=self.RETRACT_Z_HEIGHT,
            step_m=0.0005,

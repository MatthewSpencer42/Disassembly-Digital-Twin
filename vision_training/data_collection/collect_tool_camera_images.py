...
SAVE_SUBDIR = "tool_camera_sessions"


class ToolCameraImageCollector(Node):
    def __init__(self) -> None:
        super().__init__("tool_camera_image_collector")
        self.bridge = CvBridge()

        self.image_topic = "/tool_cam/image_raw"
        self.camera_info_topic = "/tool_cam/camera_info"
        ...
        self.output_dir = os.path.join(self.session_path, "tool_camera")
        ...
        self.get_logger().info("--- Tool Camera Image Collection System ---")
    ...
    def print_controls(self) -> None:
        print("\n" + "=" * 60)
        print("  TOOL CAMERA DATA COLLECTION CONTROLS")
    ...
    def save_frame(self, mode: str = "record") -> bool:
        if self.frame is None:
            if mode == "snapshot":
                self.get_logger().warn("Waiting for tool camera frame...")
            return False

        timestamp = datetime.now().strftime("%H%M%S_%f")
        file_name = f"tool_camera_{timestamp}.png"
    ...
    def display_callback(self) -> None:
        ...
        if self.frame is not None:
            ...
            cv2.putText(
                preview,
                "WAITING FOR /tool_cam/image_raw ...",
                (80, 360),
        ...
        cv2.imshow("Tool Camera Image Collection", final_frame)

...
class ExoticaDualArmPlanner:
    ...
    def __init__(self, node, hardware_type: str = "fake"):
        ...
        self._np = np
        self._exo = exo
        self._share_dir = Path(get_package_share_directory("nr_dual_arm_moveit_config"))
    ...
    def _generate_config(self) -> Path:
        ...
        urdf_xacro_path = self._share_dir / "config" / "nr_dual_arm.urdf.xacro"
        urdf_path = Path(gettempdir()) / f"nr_dual_arm_exotica_{self.hardware_type}.urdf"
        ...
        config_text = config_text.replace(
            "__SRDF_PATH__", str(self._share_dir / "config" / "nr_dual_arm.srdf")
        )
...
class ExoticaDualArmPosePlanner:
    ...
    def _generate_config(self) -> Path:
        ...
        srdf_path = self._share_dir / "config" / "nr_dual_arm.srdf"
  ...
  <UnconstrainedEndPoseProblem Name="dual_arms_stream_ik_problem">
    ...
    <Maps>
      ...
      <EffFrame Name="XARM5_TCP">
        <EndEffector>
          <Frame Link="xarm_gripper_tcp" LinkOffset="0 0 0 0 0 0 1"/>
        </EndEffector>
      </EffFrame>
...
class RemoteExoticaIKClient:
    ...
    def __init__(
        ...
        share_dir = Path(get_package_share_directory("nr_dual_arm_moveit_config"))
        ...
        if skip_ready_wait:
            ...
            self.last_error = None
            self.node.get_logger().info(
                f"[RemoteExoticaIKClient/{self.group_name}] Assuming EXOTica IK server is ready."
            )
        else:

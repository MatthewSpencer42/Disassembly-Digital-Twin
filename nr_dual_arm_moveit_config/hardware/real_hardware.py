...
XARM_GRIPPER_RAD_CLOSE = 0.0
XARM_GRIPPER_RAD_OPEN = 0.854
XARM_GRIPPER_MM_CLOSE = 0.0
XARM_GRIPPER_MM_OPEN = 86.0
...
INITIAL_POSITIONS = {
    ...
    "xarm5_joint5": 0.0,
    "xarm_gripper_right_drive_joint": XARM_GRIPPER_RAD_CLOSE,
    "uf850_joint1": 0.0,
    "uf850_joint2": 0.0,
...
class XArmGripperBridge:
    def __init__(self, arm_bridge: ArmBridge, logger):
        self.arm_bridge = arm_bridge
        self.logger = logger
        self.available = False
        self.target_width_mm = XARM_GRIPPER_MM_CLOSE
        self.shadow_width_mm = XARM_GRIPPER_MM_CLOSE
        self.pending_width_mm = None
        self.last_sent_width_mm = None
        self._log_state = {}

        if self.arm_bridge.api is None:
            self.logger.warning("xarm_gripper: xArm controller unavailable, using shadow state")
            return
    ...
    def _discover(self):
        for action_name, args in (
            ...
            try:
                code, _ = self.arm_bridge._call_api(action_name, *args)
            except Exception as exc:
                self.logger.warning(f"xarm_gripper: {action_name}{args} raised: {exc}")
                return
            if code not in (None, 0):
                self.logger.warning(f"xarm_gripper: {action_name}{args} returned SDK code {code}")
                return

        self.available = True
        self.logger.info("xarm_gripper: SDK bridge initialized via xarm5 controller")

    def set_width_mm(self, width_mm: float):
        width_mm = clamp(width_mm, XARM_GRIPPER_MM_CLOSE, XARM_GRIPPER_MM_OPEN)
        self.target_width_mm = width_mm
        self.pending_width_mm = width_mm
        if not self.available:
            self.shadow_width_mm = width_mm

    def process_pending_command(self):
        if self.pending_width_mm is None:
            return False
        if not self.available:
            self.shadow_width_mm = self.pending_width_mm
            self.pending_width_mm = None
            return False
        if self.last_sent_width_mm is not None and abs(self.pending_width_mm - self.last_sent_width_mm) < GRIPPER_WIDTH_EPS_MM:
            self.pending_width_mm = None
        ...
        try:
            ...
            if throttle(self._log_state, "command_exc", 1.0):
                self.logger.warning(f"xarm_gripper: failed to send width command: {exc}")
            return False
        ...
        if throttle(self._log_state, "command_warn", 1.0):
            self.logger.warning(
                f"xarm_gripper: set_gripper_position returned SDK code {code} for target {raw_width}"
            )
        return False

    def read_width_mm(self):
        if not self.available:
            return self.shadow_width_mm, False

        try:
            code, value = self.arm_bridge._call_api("get_gripper_position")
        except Exception as exc:
            if throttle(self._log_state, "read_exc", 2.0):
                self.logger.warning(f"xarm_gripper: failed to read width: {exc}")
            return self.shadow_width_mm, False

        if code != 0:
            if throttle(self._log_state, "read_warn", 2.0):
                self.logger.warning(f"xarm_gripper: get_gripper_position returned SDK code {code}")
            return self.shadow_width_mm, False

        try:
            width_mm = float(value) / 10.0
        except Exception:
            width_mm = self.shadow_width_mm

        self.shadow_width_mm = clamp(width_mm, XARM_GRIPPER_MM_CLOSE, XARM_GRIPPER_MM_OPEN)
        return self.shadow_width_mm, True
...
class RealHardware(Node):
    def __init__(self):
        ...
        self.xarm_gripper = XArmGripperBridge(self.xarm, self.get_logger())
        ...
        self.last_xarm_gripper_command = None
    ...
    def xarm_gripper_rad_to_width_mm(self, value: float) -> float:
        normalized = (
            clamp(value, XARM_GRIPPER_RAD_CLOSE, XARM_GRIPPER_RAD_OPEN) - XARM_GRIPPER_RAD_CLOSE
        ) / (XARM_GRIPPER_RAD_OPEN - XARM_GRIPPER_RAD_CLOSE)
        return XARM_GRIPPER_MM_OPEN + normalized * (XARM_GRIPPER_MM_CLOSE - XARM_GRIPPER_MM_OPEN)

    def xarm_gripper_width_mm_to_rad(self, width_mm: float) -> float:
        normalized = (
            clamp(width_mm, XARM_GRIPPER_MM_CLOSE, XARM_GRIPPER_MM_OPEN) - XARM_GRIPPER_MM_CLOSE
        ) / (XARM_GRIPPER_MM_OPEN - XARM_GRIPPER_MM_CLOSE)
        return XARM_GRIPPER_RAD_OPEN + normalized * (
            XARM_GRIPPER_RAD_CLOSE - XARM_GRIPPER_RAD_OPEN
        )
    ...
    def handle_joint_command(self, msg: JointState):
        ...
        if "xarm_gripper_right_drive_joint" in command_map:
            if time.time() - self.start_time < GRIPPER_COMMAND_ARM_DELAY_S:
                self.last_xarm_gripper_command = float(command_map["xarm_gripper_right_drive_joint"])
            else:
                command = float(command_map["xarm_gripper_right_drive_joint"])
                if self.last_xarm_gripper_command is None or abs(command - self.last_xarm_gripper_command) > 1e-3:
                    self.xarm_gripper.set_width_mm(self.xarm_gripper_rad_to_width_mm(command))
                    self.last_xarm_gripper_command = command
    ...
    def publish_state(self):
        ...
        xarm_gripper_width_mm, xarm_gripper_valid = self.xarm_gripper.read_width_mm()
        ...
        self.xarm_gripper.process_pending_command()
        ...
        if xarm_gripper_valid:
            live_sources.append("xarm_gripper")
        ...
        if xarm_valid and uf_valid:
            ...
            if not self.logged_waiting_for_state:
                ...
                if not xarm_gripper_valid:
                    waiting_on.append("xarm_gripper")
        ...
        xarm_gripper_position = self.xarm_gripper_width_mm_to_rad(xarm_gripper_width_mm)
        rg6_gripper_position = self.width_mm_to_rad(self.rg6.last_width_with_offset_mm)
        ...
        message.position = [
            ...
            *xarm_positions,
            xarm_gripper_position,
            *uf_positions,
            rg6_gripper_position,
        ]
        message.velocity = [
            0.0,
            *xarm_velocities,

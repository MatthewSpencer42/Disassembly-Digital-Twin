...
if not WS_ROOT or not os.path.exists(os.path.join(WS_ROOT, 'vision_training')):
    # Try the user's standard workspace location
...
if os.path.exists(WS_ROOT):
    ...
    # Check both potential .venv locations
    VENV_PATHS = [
...
DEBUG_PUBLISH_EVERY_N_FRAMES = 3
...
class AgentNode(Node):
    def __init__(self):
        ...
        self.inference_pool = ThreadPoolExecutor(max_workers=3)
        ...
        try:
            ...
            if torch.cuda.is_available():
                self.get_logger().info(f"✅ CUDA active: {torch.cuda.get_device_name(0)}")
            else:
                self.get_logger().warn("⚠️ CUDA not available. Vision inference is running on CPU.")
        except Exception as e:
            self.get_logger().warn(f"⚠️ Unable to query CUDA status: {e}")
    ...
    def _maybe_log_perf(self):
        if PERF_LOG_INTERVAL_SEC <= 0.0:
            return
        now = time.time()
        if now - self._perf_time < PERF_LOG_INTERVAL_SEC:
            return
        parts = [f"FPS={self._fps_display:.1f}"]
        for key, (total_ms, count) in self._perf_stats.items():
            avg_ms = (total_ms / count) if count else 0.0
            parts.append(f"{key.replace('_ms', '')}={avg_ms:.1f}ms")
            self._perf_stats[key] = [0.0, 0]
        self.get_logger().info("Vision perf: " + " | ".join(parts))
        self._perf_time = now
    ...
    def _poll_inference_futures(self):
        ...
        if self.sniper_future and self.sniper_future.done():
            try:
                ...
                self._record_stage_time("sniper_ms", elapsed)
            except Exception as e:
        ...
        if self.referee_future and self.referee_future.done():
            try:
                ...
                self._record_stage_time("referee_ms", elapsed)
            except Exception as e:
    ...
    def processing_loop(self):
        ...
        if self.frame_global is not None:
            # Run Global Scout asynchronously below loop rate and reuse cached detections/debug frame in between.
            if self.frame_counter % GLOBAL_SCOUT_EVERY_N_FRAMES == 0 and self.scout_future is None:
                self.scout_future = self.inference_pool.submit(self._run_scout, self.frame_global.copy())
        ...
        if self.frame_local is not None:
            ...
            if self.frame_counter % LOCAL_INFERENCE_EVERY_N_FRAMES == 0 and self.sniper_future is None:
                self.sniper_future = self.inference_pool.submit(self._run_sniper, self.frame_local.copy())
            if self.frame_counter % REFEREE_EVERY_N_FRAMES == 0 and self.referee_future is None:
                self.referee_future = self.inference_pool.submit(self._run_referee, self.frame_local.copy())

            sniper_data.update(self.last_sniper_data)
        ...
        if self.frame_counter % DEBUG_PUBLISH_EVERY_N_FRAMES == 0:
            try:
                ...
                self._record_stage_time("debug_ms", time.perf_counter() - debug_t0)
...
def main(args=None):
    ...
    try:
        ...
        node.inference_pool.shutdown(wait=False, cancel_futures=True)
        node.destroy_node()
        rclpy.shutdown()
